...
> Last updated: 2026-07-05 — **Visual/interaction polish pass (landing + app surfaces).** Hero: pill-fact row removed (4-element stack: badge, headline, sub, CTAs); new spring-lagged `CursorGlow` layer (`components/effects/cursor-glow.tsx`, violet orb in light, faint neutral spotlight in dark, ambient drift on touch, static under reduced motion) layered over the WebGL fluid sim, plus a `.noise` grain overlay; display scale raised to clamp(3.25rem, 6.5vw, 6rem). Section headers de-templated: the identical pill-eyebrow recipe removed everywhere except the hero badge; LiveExample + Features headers are now left-aligned. Features rebuilt as an asymmetric bento (col spans 2+1 / 1+2 / 2+1) using new `SpotlightCard` (cursor-tracked radial highlight via CSS vars, no re-renders). LiveExample: `edge-hairline` framing, score bars animate on scroll, count-up score chips, cycle-timer bar + larger dot hit areas on the sample panel. HowItWorks: ghost display numerals + self-drawing connector. Pricing: featured-tier elevation (lg:-mt-4), hover lift, staggered feature reveal (data untouched). Buttons: press scale + hover lift; `lift` shadow re-tinted pure violet (was part orange). Navbar scroll state now uses framer `useScroll` (raw scroll listener removed); signup CTA label unified to "Start screening free" everywhere. Fraunces font removed (unused). Dark-mode fixes: upload zone / upload form / processing state literals tokenized so dark stays neutral. Copy: all user-visible em dashes removed (incl. score placeholder now "-"). New: `PRODUCT.md` (gitignored, impeccable-skill context). Build + typecheck clean.
> Prior (2026-06-27) — **Sample Output relocated.** The "Sample Output" card (`LiveExamplePanel`) moved off the results page (`app/dashboard/results/[id]/page.tsx`, now single-column `max-w-5xl`) onto the landing page. New `landing/live-example.tsx` section places the ranked shortlist on the LEFT and the Sample Output write-up on the RIGHT, side by side, between Hero and Features. The hero's floating preview card was folded into that section so the ranking list isn't duplicated.
